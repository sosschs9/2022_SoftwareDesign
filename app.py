...
def writing_post(userKey):
    ...
    if request.method == 'POST':
        ...
        return render_template('Post/newPost.html', checkType=curUser.checkType, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def view_post(post_id):
    ...
    return render_template('Post/readPost.html', checkType=curUser.checkType, len=len(result['commentList']), userID=curUser.userID, isLogin=curUser.isLogin, userKey=curUser.userKey, post=post)
...
def modify_post(post_id):
    ...
    return render_template('Post/modify.html', checkType=curUser.checkType, userID=curUser.userID, isLogin=curUser.isLogin, userKey=curUser.userKey, post=post)
...
def request_matching(userKey):
    ...
    return render_template('Matching/newMatching.html', checkType=curUser.checkType, isLogin=curUser.isLogin, userKey=userKey)
...
def accompany_matching_key(userKey):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('check_request_userKey', helpType=HelperType.ACCOMPANY, matchingID=matchingID, userKey=curUser.userKey))
...
def accompany_matching():
    ...
    return render_template("Matching/accompanyMatching.html", checkType=curUser.checkType, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def counsel_matching_key(userKey):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('check_request_userKey', helpType=HelperType.COUNSEL, matchingID=matchingID, userKey=curUser.userKey))
    return redirect(url_for('counsel_matching'))

@app.route(path+'/counsel', methods=['GET', 'POST'])
def counsel_matching():
    return render_template("Matching/counselMatching.html", checkType=curUser.checkType, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def safe_matching_key(userKey):
    ...
    if request.method == 'POST':
        ...
        return redirect(url_for('check_request_userKey', helpType=HelperType.SAFETYCHECK, matchingID=matchingID, userKey=curUser.userKey))
    return redirect(url_for('safe_matching')) 

@app.route(path+'/safe', methods=['GET', 'POST'])
def safe_matching():
    return render_template("Matching/safeMatching.html", checkType=curUser.checkType, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def check_request(helpType, matchingID):
    ...
    return render_template('check.html', checkType=curUser.checkType, matchingID=matchingID, check=check, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def matching_board(helperType, page):
    ...
    return render_template('Matching/matchingBoard.html', checkType=curUser.checkType,
                           page_list=page_list, max_page=max_page, this_page=page, start=block_start, end=block_end, isLogin=curUser.isLogin, userKey=curUser.userKey)
...
def view_matching(matching_id):
    ...
    return render_template('Matching/readMatching.html', checkType=curUser.checkType, helpType=curUser.checkType, help=help, matching=result, isLogin=curUser.isLogin, userKey=curUser.userKey)
